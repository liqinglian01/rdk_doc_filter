...
页面顶部可通过「版本」「产品」筛选与正文中的条件块联动（URL 会带上 `?v=`、`?p=`）。共用段落写在条件块外，只维护一份；差异内容用下列指令包裹（`versions` / `products` 为英文逗号分隔，可只写其一；不写或 `*` 表示该维度不限制）。

:::doc_scope{versions="3.0.0" products="RDK X3"}

**示例：**当前选择为 3.0.0 且产品为 RDK X3 时显示本段。

:::

:::doc_scope{versions="3.5.0" products="RDK X5"}

**示例：**当前选择为 3.5.0 且产品为 RDK X5 时显示本段。
